...
&& echo $TS_URL \
&& export WGET_URL="$(curl -s $TS_URL | grep -o -E 'http.+\w+' | grep -i "$(uname -s)" | grep -i "$(uname -r | sed 's/.*-//')")" \
&& echo $WGET_URL \
&& mkdir -p /TS && chmod -R 666 /TS \
&& mkdir -p $TS_CONF_PATH && chmod -R 666 $TS_CONF_PATH \
&& wget --no-verbose --no-check-certificate --user-agent="$USER_AGENT" --output-document=/TS/TorrServer --tries=3 $WGET_URL \
&& echo "done"
